...
.factory('MembersFactory', function ($firebaseArray, $q, myCache, $timeout) {
        ...
        var fbs = firebase.storage().ref();
        var ref = fb.child("users");
        var mRef = fb.child("members");
        var iRef = fbs.child("images");
        return {
            ref: function () {
                return ref;
            },
            ...
            getMember: function (shopid) {
                var deferred = $q.defer();
                var memberRef = mRef.child(shopid);
...
.factory('MasterFactory', function ($firebaseArray, $q, myCache, CurrentUserService, $timeout) {
        var fb = firebase.database().ref();
        var ref = {};
        var wRef = {};
        var pRef = {};
        ...
        var thisShopId = myCache.get('thisMemberId');
        return {
            ref: function () {
                ref = fb.child("members").child(thisShopId).child("customers");
                return ref;
            },
            wRef: function () {
                wRef = fb.child("members").child(thisShopId).child("warehouses");
                return wRef;
            },
            pRef: function () {
                pRef = fb.child("members").child(thisShopId).child("products");
                return pRef;
            },
            ...
            getWarehouses: function () {
                ref = fb.child("members").child(thisShopId).child("warehouses");
                warehousesRef = $firebaseArray(ref);
                return warehousesRef;
            },
            getProducts: function (id) {
                ref = fb.child("members").child(thisShopId).child("products");
            ...
            getProductById: function (productid) {
                var deferred = $q.defer();
                ref = fb.child("members").child(thisShopId).child("products").child(productid);
                ref.on("value", function(snap) {
                    deferred.resolve(snap.val());
                });
                return deferred.promise;
...
.factory('TransactionFactory', function ($firebaseArray, $q, myCache, CurrentUserService, $timeout) {
        ...
        return {
            ...
            getTransactions: function (data) {
                ref = fb.child("members").child(thisShopId).child("transactions").orderByChild("dateCreated").startAt(data);
...
.service("CurrentProductService", function () {
        var thisProduct = this;
        thisProduct.updateProduct = function (product) {
            ...
            this.productSellPrize = product.productSellPrize;
            this.stockGudang = product.stockGudang;
            this.stockRumah = product.stockRumah;
            this.stockToko = product.stockToko;
